...
def conv2d(x, filters, kernel=4, strides=1, norm=True, drop=0.0):
...
def deconv2d(x, contracting_path, filters, kernel=4, norm=True, drop=0.0):
    x = UpSampling2D(2)(x)
...
def create_gens(img_shape):
    ...
    def back_end(x, shortcut_path, filters):
        ...
        return y, z
    def u_net(x, filters):
        c1 = conv2d(x, filters, strides=2, norm=False)
        c2 = conv2d(c1, filters*2, strides=2)
        c3 = conv2d(c2, filters*4, strides=2)
        x = conv2d(c3, filters*8, strides=2)
        x = deconv2d(x, c3, filters*4)
        x = deconv2d(x, c2, filters*2)
        x = deconv2d(x, c1, filters)
        z = UpSampling2D(2)(x)
        y = Conv2D(img_shape[-1], 4, padding='same', activation='tanh')(z)
        return y, z
